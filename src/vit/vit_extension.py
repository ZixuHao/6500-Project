...
def main():
    ...
    cfg = RunConfig(**vars(args), device=("cuda" if torch.cuda.is_available() else "cpu"))
    ...
    for epoch in range(cfg.epochs):
        ...
        val_metrics = evaluate(
            model, val_loader, criterion, cfg.device, class_names,
            save_mistakes_to=os.path.join(cfg.output_dir, "mistakes_preview.json") if epoch == cfg.epochs - 1 else None,
        )
    ...
    save_json(os.path.join(cfg.output_dir, "metrics.json"), summary)
    print(f"Saved metrics to {cfg.output_dir}/metrics.json")
